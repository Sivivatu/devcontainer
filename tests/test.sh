...
# =============================================================================
# Builds the base image, verifies all expected tools are present, then tests
# each feature by layering it on top of the base.
#
# Usage:
...
test_base_tools() {
    ...
    # Bun
    assert_cmd "${BASE_IMAGE}" "bun is installed" bun --version

    # DuckDB
    assert_cmd "${BASE_IMAGE}" "duckdb CLI is installed" duckdb --version
    assert_cmd "${BASE_IMAGE}" "duckdb Python package" python3 -c "import duckdb; print(duckdb.__version__)"
}
...
EOF
    ...
    assert_cmd "${tag}" "bun still works" bun --version
    assert_cmd "${tag}" "duckdb still works" duckdb --version
...
main() {
    ...
    case "${target}" in
        ...
        all)
            ...
            ;;
        *)
            echo "Usage: $0 [all|base|features|dbt-duckdb|k8s-tools|postgres-client]"
            exit 1
            ;;
    esac
